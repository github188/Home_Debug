...
// 外部全局变量
//-----------------------------------------------------------------------------
extern unsigned int T0Counter2; 
extern xdata unsigned char LEDOut1 ;   
extern xdata unsigned char LedPort1;
extern xdata unsigned int  T0Counter3; 
//-----------------------------------------------------------------------------
// 全局变量
//-----------------------------------------------------------------------------
//xdata CAN2RECBUF1    CAN2PWMRXbuf;
//bit Can2NewData1=NO;
//bit Can2NewData2=NO;
...
extern void can2buf1_process(void);
//extern void LedBlink(unsigned char num) ; 
...
//CAN控制器SJA1000T产生的外部中断INT0 ，高优先级 
...
{   
   ...
   {     
      ...
      if( (address.value==71)&&((FIFO16&0x40)!=0x40))   //相同ID号 ,非远程帧
	  { 
	  	LED4=!LED4;//接受数据指示
 //        can2_receive (CAN2PWMRXbuf.buf);
 //        can2buf1_process();
         //Can2NewData1=YES; 	 
      }
  
      CAN_CMR=0x04;  //释放接收缓冲器
      ////LedBlink(8);   //CAN2接收指示灯闪烁
//	  LedPort1&=0xFE;
//	  LEDOut1 = LedPort1 ;   //故障指示灭 
	}
