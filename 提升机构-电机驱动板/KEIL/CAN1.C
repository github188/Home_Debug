...
xdata volatile unsigned char CANaddress1 = 0;
xdata volatile unsigned char CANINDEX = 0;
xdata volatile unsigned char CAN1FaultCounter = 0;
...
void can1_transmit(char MsgNum, unsigned char *buf )
{
    ...
    CAN0ADR = TRANSREQ1;
    delay_ms(100);
    if ((CAN0DAT & (0x0001 << (MsgNum - 1)) ) != 0) //检查消息对象的传输任务是否完成
    {
        CAN1FaultCounter = CAN1FaultCounter + 1;
        SFRPAGE = SFRPAGE_SAVE;             // Restore SFR page
        EA = 1;
        return; //延时30ms  超时退出
    }
...
////////////////////////////////////////////////////////////////////////////////
void CAN1_ISR (void) interrupt 19
{
    ...
    if ((status & 0x10) != 0)	// RxOk is set, interrupt caused by reception
    {
        ...
        {
            WDTCN = 0xA5;
            can1_receive (RX_MSGNUM_ZKB, CANRXBUF_ZKB.buf); //接收并存储消息
            ...
            case 0xFE:
                sum = 0;
                for(isum = 0; isum < 7; isum++)
                {
                    sum = sum + CANRXBUF_ZKB.buf[isum];
                }
                if(CANRXBUF_ZKB.total_ctl_buf.checkout == sum)
                {
                ...
                //added begin
                //借用任务自动分配函数，实现翻转输送线翻转
                //手段：设定不同的起始步骤值system_crtl.AUTOsystem_command
            case 0xB0:
                sum = 0;
                for(isum = 0; isum < 7; isum++)
                {
    ...
    if ((status & 0x08) != 0)	// TxOk is set, interrupt caused by transmision
    {
        CAN0STA = (CAN0STA & 0xF7) | 0x07;    // Reset TxOk, set LEC to NoChange
    }
    if (((status & 0x07) != 0) && ((status & 0x07) != 7))	// Error interrupt, LEC changed
    {
        /* error handling ? */
